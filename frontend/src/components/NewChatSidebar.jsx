...
const NewChatSidebar = ({ isOpen, onClose, setSelectedUser }) => {
  ...
  return (
    <div className={`absolute top-0 left-0 h-full w-full bg-white dark:bg-gray-900 z-20 flex flex-col transition-transform duration-300 ease-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      {/* Header */}
      ...
      {/* Search Input */}
      <div className="bg-white dark:bg-gray-900 p-2 border-b border-gray-200 dark:border-gray-700 shrink-0">
        <div className="bg-[#f0f2f5] dark:bg-gray-800 rounded-lg flex items-center px-3 py-1.5">
          ...
          results.map((user) => {
            ...
            return (
              <div 
                ...
                className="flex items-center px-3 cursor-pointer hover:bg-[#f5f6f6] dark:hover:bg-gray-800 transition-colors group"
              >
                ...
                <div className="flex-1 min-w-0 border-b border-gray-100 dark:border-gray-800 h-full py-3 pr-2 flex flex-col justify-center">
                  <h2 className="font-normal text-[17px] text-[#111b21] dark:text-gray-100 truncate">{user.name}</h2>
                  <div className="flex text-[13px] text-[#667781] dark:text-gray-400 gap-2 mt-0.5">
